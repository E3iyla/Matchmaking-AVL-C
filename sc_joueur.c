...
int main(){
    ...
    t_joueur* j2 = nouveau_joueur("JObi-Wan Kenobi", 2000, 1);
    t_joueur* j3 = nouveau_joueur("C-3PO", 500, 1);
    t_joueur* j4 = nouveau_joueur("Yoda", 2500, 1);
    t_joueur* j5 = nouveau_joueur("Luke Skywalker", 800, 1); // c est quand il debute 

    // on les mets dans les arbres 
    ...
    // affichage
    printf("Affichage trie par ELO du plus faible au plus fort\n");
    afficher_conteneur(mon_arbre);
    // on retirer les deux joueurs avec l'ELO le plus faible
    t_element joueur_A_element = NULL;
    t_element joueur_B_element = NULL;

    printf("\nResultat du Matchmaking V1 \n");
    ...
    if (retirer_conteneur(&mon_arbre, &joueur_A_element)) {
        ...
        if(retirer_conteneur(&mon_arbre, &joueur_B_element)) {
            ...
            printf("Match trouve entre deux joueurs qui attendaient :\n");
            printf("A :  "); afficher_joueur(jA); printf("  Attente: %.0f sec\n", temps_ecoule_sec(jA));
            printf("B :  "); afficher_joueur(jB); printf("  Attente: %.0f sec\n", temps_ecoule_sec(jB));
    ...
    printf("\nLes joueur restant apres le match \n");
    afficher_conteneur(mon_arbre);
